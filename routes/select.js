...
router.post("/login", (req, res) => {
  const { id, password, fcm_token } = req.body;
...
router.post("/robots", (req, res) => {
  ...
  connection.query(getfa_id_query, [username], (error, results) => {
    if (error) {
      return res
        .status(500)
        .json({ message: "유저를 찾는 도중 오류가 났습니다", error });
    }
    if (results.length === 0) {
    ...
    connection.query(query, [facility_id], (error, results) => {
      if (error) {
        return res
          .status(500)
          .json({ message: "로봇을 찾는 도중 에러가 났습니다", error });
      }
      if (results.length === 0) {
        return res
          .status(404)
          .json({ message: "소유한 로봇을 찾을 수 없습니다" });
      }
      res.json(results);
    });
  });
  ...
  connection.query(getfa_id_query, [username], (error, results) => {
    ...
    connection.query(query, [facility_id], (error, results) => {
      if (error) {
        return res
          .status(500)
          .json({ message: "알림을 찾는 도중 에러가 났습니다", error });
      }
      if (results.length === 0) {
...
router.post("/web-login", (req, res) => {
  ...
  console.log(req.body);
  const checkUserQuery =
    "SELECT * FROM admin WHERE ad_email = ? AND ad_password = ?";
  ...
  connection.query(checkUserQuery, [email, password], (error, results) => {
    ...
    if (results.length > 0) {
      ...
      res
        .status(200)
        .json({ userEmail: results[0].ad_email, userName: results[0].ad_name });
    } else {
      res.status(401).send("이메일 또는 비밀번호가 틀렸습니다.");
      console.log("err");
    }
